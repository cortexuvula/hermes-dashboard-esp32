...
- **Timing (A5)**: relay worst-case response ≤ 4 s — status fetch budget 3 s, usage refreshed on a background thread (a stalled usage upstream costs ~1–5 ms). Status failure → 502 + JSON error (board renders OFFLINE).
- **Null semantics (A6)**: unavailable usage data is JSON null (never omitted, never zeroed); new `usage_age_s`, `generated_at`, `schema: 2`; `components` null (never fabricated) when upstream omits it.
